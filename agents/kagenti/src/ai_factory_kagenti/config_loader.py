...
def _resolve_content_dir() -> pathlib.Path:
    """Resolve the content directory from env or source tree layout."""
    if os.environ.get("CONTENT_DIR"):
        return pathlib.Path(os.environ["CONTENT_DIR"])
    # When running from source: agents/kagenti/src/ai_factory_kagenti/ -> agents/ -> project root
...
def get_content_api_url() -> str:
    """Return the content API URL from the Configuration singleton."""
    from .configuration import Configuration
    config = Configuration()
    return config.content_api_url
